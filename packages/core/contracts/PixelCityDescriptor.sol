...
contract PixelCityDescriptor is IPixelCityDescriptor, Ownable {
  ...
  function _randomize(uint256 _nonce) internal view returns (uint256) {
    return
      uint256(
        keccak256(
          abi.encodePacked(
  ...
  function _genAccessory(uint256 _seed) internal view returns (uint256) {
    return _seed << 8 % accessories.length;
  }

  function _genFace(uint256 _seed) internal view returns (uint256) {
    return _seed << 16 % faces.length;
  }

  function _genTee(uint256 _seed) internal view returns (uint256) {
    return _seed << 32 % tees.length;
  }

  function _genHead(uint256 _seed) internal view returns (uint256) {
    return _seed << 64 % heads.length;
  }
  ...
  {
    uint256 seed = _randomize(_tokenId);
